package com.irfan06.bloodbankbyirfan;
...
import com.irfan06.bloodbankbyirfan.adapters.UserAdapter;
import com.irfan06.bloodbankbyirfan.model.User;
...
public class DisplayDonorsActivity extends AppCompatActivity {
    ...
    void initializeComponents() {
        ...
        dropDownBG = findViewById(R.id.bgDropDonors);
        dropDownBG.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_dropdown_item_1line, getResources().getStringArray(R.array.blood_groups)));
        ...
        districtFilter.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                updateList(s.toString(),dropDownBG.getSelectedItem().toString());
            }
        });

        dropDownBG.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                updateList(districtFilter.getText().toString(),bloodGroups[i]);
            }
    ...
    private void updateList(String district, String bloodGroup) {
        temp.clear();
        for( User v : users){
            if(v.getDistrict().toUpperCase().contains(district.toUpperCase())||district.equalsIgnoreCase("ALL")) {
                if(v.getBloodGroup().equalsIgnoreCase(bloodGroup) || bloodGroup.equalsIgnoreCase("ALL")) {
                    temp.add(v);
                }
            }
        }
    ...
    private void getDonors() {
        FirebaseDatabase.getInstance().getReference("Donors").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                updateList(districtFilter.getText().toString(),dropDownBG.getSelectedItem().toString());
                filterList();
            }
